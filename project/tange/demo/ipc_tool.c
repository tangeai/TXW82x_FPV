...
#define IPCPARAMF_TAG 0x464d5258U /* GCC multi-character tag FMRX, persistent format unchanged */
struct IPrmfHead {
	uint32_t tag;
...
int write_flash(int address, const uint8_t *buff, int len)
{
	...
	spi_nor_write(&flash0, address, (uint8_t *)buff, len);
	spi_nor_close(&flash0);
	return len;
}
/* return: >0 实际复制出的字节数
...
static int write_params(const SIMPLEBUFFER *bf)
{
#ifdef PARAM_IN_MEM
	if (!_data) {
		_data = (uint8_t*)os_calloc_psram(1, 4096);
