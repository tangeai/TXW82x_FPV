...
/* 卡录像 H264 framebuff stype: 按应用模式、芯片和主/子码流选择.
 ...
 *   828 双目主码流: 两路 1280x720 纵向拼接, stype=FSTYPE_H264_VPP_DATA0 (1280x1440)
 *   828 双目子码流: 拼接后 gen420, stype=FSTYPE_H264_GEN420_DATA (640x720)
...
/* MP4 fast-start 文件的 moov/mdhd 位于文件头. 扫描录像列表时只读前 512B,
 * 不创建 demux、不遍历 mdat, 用很小的 I/O 代价取得真实媒体时长. */
#define REC_MP4_MDHD_SCAN_SIZE  512U
...
/* 读取 mdhd.duration / timescale 并向上取整到秒.
 * fallback_duration 来自文件名, 同时也是上限: 正常文件因 GOP 收尾可能比计划
 * 时长略长, 列表仍保持文件名时长; 只有断电等造成的短文件才缩短时间轴. */
static uint16_t rec_mp4_media_duration(const char *path, uint16_t fallback_duration)
{
    uint8_t head[REC_MP4_MDHD_SCAN_SIZE];
    void *fp = osal_fopen(path, "rb");
    if (!fp) return fallback_duration;

    int got = osal_fread(head, 1, sizeof(head), fp);
    osal_fclose(fp);
    if (got < 28) return fallback_duration;

    /* i 指向 box type 的 'm', i-4 是 box size. */
    for (int i = 4; i + 24 <= got; i++) {
        if (head[i] != 'm' || head[i + 1] != 'd' ||
            head[i + 2] != 'h' || head[i + 3] != 'd') {
            continue;
        }

        uint32_t box_size = rec_mp4_be32(&head[i - 4]);
        uint8_t version = head[i + 4];
        uint32_t timescale = 0;
        uint64_t duration = 0;

        if (version == 0) {
            if (box_size < 32 || i + 24 > got) continue;
            timescale = rec_mp4_be32(&head[i + 16]);
            duration  = rec_mp4_be32(&head[i + 20]);
        } else if (version == 1) {
            if (box_size < 44 || i + 36 > got) continue;
            timescale = rec_mp4_be32(&head[i + 24]);
            duration  = rec_mp4_be64(&head[i + 28]);
        } else {
            continue;
        }

        if (timescale == 0 || duration == 0) return fallback_duration;

        uint64_t seconds = (duration + timescale - 1U) / timescale;
        if (fallback_duration != 0 && seconds > fallback_duration)
 ...
 *     media_duration 利用原结构体的对齐空洞, 新结构仍为 12B. */
typedef struct {
    uint32_t t_start;       /* UTC 秒 */
    uint16_t duration;      /* 文件名里的计划秒数, 仅用于重建真实文件名 */
    uint16_t media_duration;/* mdhd 真实秒数, 用于列表和回放时间轴 */
    uint8_t  event;
                        ...
                        uint8_t read_media_duration)
{
    ...
    *out_cnt = 0;
    ...
    while ((fno = osal_readdir(d)) != NULL && idx < cnt) {
        ...
        arr[idx].media_duration = dur;
        if (read_media_duration) {
            char full_path[96];
            os_snprintf(full_path, sizeof(full_path), "%s/%s", sub_path, fname);
            arr[idx].media_duration = rec_mp4_media_duration(full_path, dur);
            if (arr[idx].media_duration != dur) {
                os_printf(KERN_INFO "rec_scan: correct duration %s %us -> %us (mdhd)\n",
                          fname, (unsigned)dur,
                          (unsigned)arr[idx].media_duration);
            }
        }
        arr[idx].event    = ev;
        arr[idx].hh       = hh;
                            ...
                            char *next_fname, uint32_t *next_t0)
{
    ...
    uint32_t cnt = 0;
    if (scan_day_dir(date_dir, &arr, &cnt, 0) != 0 || cnt == 0) {
        if (arr) RP_FREE(arr);
        return -1;
    }
    int  ret = -1;
...
static int rec_recycle_oldest_locked(void)
{
    ...
    if (scan_day_dir(oldest_dir, &arr, &cnt, 0) == 0 && cnt > 0) {
...
void sd_format_handle(void*arg)
{
    ...
    /* 43482 的 FatFs 使用四参数接口。opt=NULL 等价于默认参数
     * { FM_ANY, 0, 0, 0, 0 }，继续自动选择 FAT 类型和簇大小。 */
    FRESULT res = f_mkfs("0:", NULL, work, (UINT)sizeof(work));
    if (res != FR_OK) {
    ...
    os_printf(KERN_INFO "sd_format: mkfs done\n");

    /* 重建 REC 根目录 */
    FRESULT mkdir_res = osal_fmkdir(REC_ROOT_PATH);
    if (mkdir_res != FR_OK && mkdir_res != FR_EXIST) {
        os_printf(KERN_ERR "sd_format: mkdir %s failed, res=%d\n",
                  REC_ROOT_PATH, mkdir_res);
        ret = -1;
    }

    /* 43482 配置 FF_FS_NOFSINFO=1: 格式化/重新挂载后的第一次 f_getfree()
     * 会完整扫描 FAT 来重建 free_clst 缓存. 32GB 卡实测超过 1 秒.
     *
     * 如果先恢复录像, mp4_encode_thread 会在 rec_create_file_cb 中先抢到
     * FatFs 卷锁执行这次扫描; APP 收到格式化成功后立即查询容量时只能等待
     * FF_FS_TIMEOUT(1000 tick), 最终得到 FR_TIMEOUT(15) 和 0/0 MB.
     *
     * 因此必须在录像尚未重启、g_sd_formatting 仍有效时主动完成首次扫描.
     * 后续查询直接使用 fs->free_clst, 不再和新录像线程争抢长时间卷锁.
     * 少量非录像 SD 操作仍可能并发, FR_TIMEOUT 时最多重试 3 次. */
    uint32_t format_total_mb = 0;
    uint32_t format_free_mb  = 0;
    if (ret == 0) {
        int cap_ok = 0;
        for (int attempt = 0; attempt < 3; attempt++) {
            if (sd_get_capacity(&format_total_mb, &format_free_mb) == 0 &&
                format_total_mb > 0) {
                cap_ok = 1;
                break;
            }
            if (attempt < 2) os_sleep_ms(100);
        }
        if (!cap_ok) {
            os_printf(KERN_ERR "sd_format: capacity warmup failed\n");
            ret = -1;
        } else {
            os_printf(KERN_INFO "sd_format: done, capacity=%u/%u MB (free/total)\n",
                      (unsigned)format_free_mb, (unsigned)format_total_mb);
        }
    }
     ...
     * 只有格式化结果和容量校验都成功时才清 SD 故障标志. 即使容量校验失败,
     * 仍恢复用户原录像模式, 让既有自恢复机制继续探测卡状态. */
    g_rec_mode = saved_mode;
    g_sd_formatting = 0;
    if (ret == 0) {
...
int _rp_record_start(uint8_t event_type)
{
     ...
     *   REC_STREAM_TYPE=0 → 主码流 (双目: VPP_DATA0/1280x1440)
     *   REC_STREAM_TYPE=1 → 子码流 (双目: GEN420_DATA/640x720)
     * rec_time 单位 分钟 */
    uint8_t rec_min = 1;
                             ...
                             SAvExEvent *out_arr, int out_cap)
{
    ...
    for (uint32_t i = 0; i < day_cnt; i++) {
        ...
        uint32_t ft1 = ft0 + day_arr[i].media_duration;
        ...
        if (out_idx >= 0) {
            ...
            if (out_arr) {
                ...
                prev_end   = prev_t0 + day_arr[i - 1].media_duration;
        ...
        if (out_arr) {
            ...
            out_arr[out_idx].file_len = day_arr[i].media_duration;
...
int rec_list_get(uint32_t t_start, uint32_t t_end, SAvExEvent **out_items)
{
    ...
    *out_items = NULL;
    ...
    uint32_t     day_cnt = 0;
    if (scan_day_dir(date_dir, &day_arr, &day_cnt, 1) != 0 || day_cnt == 0) {
                          ...
                          uint32_t *out_t0)
{
    ...
    scan_item_t *arr = NULL; uint32_t cnt = 0;
    int sret = scan_day_dir(date_dir, &arr, &cnt, 1);
    if (sret != 0 || cnt == 0) {
    ...
    for (uint32_t i = 0; i < cnt; i++) {
        ...
        uint32_t t1 = t0 + arr[i].media_duration;
    ...
    if (found < 0) {
        os_printf(KERN_WARNING "pb_locate: no file after t_seek=%u in %s "
                  ...
                  t_seek, date_dir, cnt,
                  arr[cnt-1].t_start,
                  arr[cnt-1].t_start + arr[cnt-1].media_duration);
        RP_FREE(arr);
        return -1;
    }
