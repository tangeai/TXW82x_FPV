...
int on_talkback_start()
{
    ...
    gy_Speaker_en(1);
#endif
    if (__talk_inited && talk_pump_hdl) return 0;
    /* A previous stop is asynchronous. Wait before re-enabling its exit flag. */
    for (int i = 0; talk_pump_hdl && i < 100; ++i) os_sleep_ms(5);
    if (talk_pump_hdl) return -1;
    talk_ring_head = 0;
    talk_ring_tail = 0;
    __talk_inited = 1;
...
int talkback(TCMEDIA at, const uint8_t *audio, int len)
{
    ...
    if (at != TCMEDIA_AUDIO_G711A || !audio || len <= 0) return -1;
