...
//-----------------------------------------------------------
#include "osal/work.h"
int32 wakeup_me(struct os_work *work)
{
	printf("set PM_ALLON\n");
	TciSetPowerMode(PM_ALLON);
	return 0;
}
...
//int on_qrcode_start() { return -1; }
//int on_get_y_data(uint8_t **ppYBuff, int *width, int *height) { return -1; }
//void on_qrcode_end(uint8_t *pYBuff) { }
...
{
    /* RTC register programming remains the vendor integration hook. */
#if 0
...
int request_iframe_ex(int channel, int vstream)
{
    ...
    if (channel != 0 || vstream < 0 || vstream > 1) return -1;
...
int Handle_P2p_Cmd(p2phandle_t handle, int cmd, const void *buf, int size)
{
	switch(cmd)
	{
		case TCI_CMD_LISTEVENT_REQ:
		{
            ...
            if (!buf || size < (int)sizeof(Tcis_ExListEventReq)) {
                Tcis_ExListEventResp end = {0};
                end.endflag = 1;
                TciSendCmdResp(handle, TCI_CMD_LISTEVENT_RESP, &end, sizeof(end));
                break;
            }
        ...
        case TCI_CMD_LIST_RECORDDAYS:
        {
            ...
            if (!resp) {
                ...
                Tcis_DaysList empty = {0};
                TciSendCmdResp(handle, TCI_CMD_LIST_RECORDDAYS, &empty, sizeof(empty));
                break;
            }
		...
		case TCI_CMD_RECORD_PLAYCONTROL:
		{
            ...
            Tcis_PlayRecordResp res = {0};
            if (!buf || size < (int)sizeof(*p)) {
                res.result = -1;
                TciSendCmdResp(handle, TCI_CMD_RECORD_PLAYCONTROL_RESP, &res, sizeof(res));
                break;
            }
			...
			if(p->command == TCIC_RECORD_PLAY_START) {
				...
				res.result = pb_set_forward(p->Param);
            }
            else
                res.result = -3;
        ...
        case TCI_CMD_FORMATEXTSTORAGE_REQ:
            {
                ...
                const Tcis_FormatExtStorageReq *req = (const Tcis_FormatExtStorageReq *)buf;
                if (!req || size < (int)sizeof(*req) || req->storage != 0) {
                    Tcis_FormatExtStorageResp resp = {0};
                    if (req && size >= (int)sizeof(*req)) resp.storage = req->storage;
                    resp.result = -1;
                    TciSendCmdResp(handle, TCI_CMD_FORMATEXTSTORAGE_RESP, &resp, sizeof(resp));
                } else {
                    sd_format(handle); /* drain workers, format SD, resume recording */
                }
            }
            break;
        case TCI_CMD_GETRECORD_REQ:
        {
            ...
            resp.recordStream = REC_STREAM_TYPE;
		...
		case TCI_CMD_SETRECORD_REQ:
		{
            ...
            Tcis_SetRecordResp resp = {0};
            if (!buf || size < (int)sizeof(*req)) {
                resp.result = 1;
                TciSendCmdResp(handle, TCI_CMD_SETRECORD_RESP, &resp, sizeof(resp));
                break;
            }
...
{
    uint32_t last_cnt = 0;
    uint32_t last_rec_md_cnt = vpp_md_cnt;
    ...
    while(1){
        ...
        /* Card event recording is independent of cloud reporting/snapshot success. */
        uint32_t rec_md_cnt = vpp_md_cnt;
        if (rec_md_cnt != last_rec_md_cnt) {
            last_rec_md_cnt = rec_md_cnt;
            if (rec_get_mode() != REC_MODE_OFF)
        ...
        if(!ipc_inited){
            ...
            if(ret == 0){
                ...
                evtp.status = 1;
                evtp.jpg_pic = (char *)md_jpg_out_data;
...
//                    //模拟连续触发事件，30秒内不会上报重复事件，但是可以延长云存储录像时长
//                    int i;
//                    for(i=0; i < 3; i++){
//                        os_sleep_ms(4000);
//                        memset(&evtp, 0x0, sizeof(evtp));
...
//                    }
        ...
        if(call_test_flag){
            ...
            if(ret == 0){
                ...
                evtp.tHappen = time(NULL);
                evtp.status = 1;
                evtp.jpg_pic = (char *)call_jpg_out_data;
                evtp.pic_len = jpg_len;
                evtp.evtp_flags = 0;
                ret = TciSetEventEx(&evtp);
                if(ret != 0){
...
void IpcStep2(void*arg)
{
    ...
    unsigned cloud_cache_szie = 200*1024; //置0则不开启云存储
