...
unsigned char *qr_binarize_ex(const unsigned char *_img,
  ...
  if(_width>0&&_height>0){
    ...
    int            mask_internal;
    int            sums_internal;
    /* use the caller's mask buffer only if it is large enough */
    mask_internal=(mask==NULL||_mask_size<(unsigned)(_width*_height));
    if(mask_internal){
      mask=(unsigned char *)zbar_malloc(_width*_height*sizeof(*mask));
    ...
    windh=1<<logwindh;
    col_sums=_col_sums;
    /* use the caller's sums buffer only if it is large enough */
    sums_internal=(col_sums==NULL||_col_sums_size<(unsigned)_width);
    if(sums_internal){
      col_sums=(unsigned *)zbar_malloc(_width*sizeof(*col_sums));
      if(col_sums==NULL){
        if(mask_internal)zbar_free(mask);
        return NULL;
      }
    }
    /*Initialize sums down each column.*/
    ...
    if(sums_internal)zbar_free(col_sums);
  }
#if defined(QR_DEBUG)
  {
    FILE *fout;
...
unsigned char *qr_binarize(const unsigned char *_img,int _width,int _height){
  return qr_binarize_ex(_img,NULL,0,NULL,0,_width,_height);
}
#endif
