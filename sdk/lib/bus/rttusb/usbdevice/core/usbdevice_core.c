...
static rt_err_t _get_config_descriptor(struct udevice* device, ureq_t setup)
{
    ...
    /* modify config desc here, device_is_hs set & endpoint reinit when RESET  */
    ...
    switch(setup->bRequest)
    {
        ...
        switch(setup->wIndex)
        {
            case 0x04:
                ...
                if(usb_comp_id_desc != RT_NULL)
                {
                    rt_free(usb_comp_id_desc);
                    usb_comp_id_desc = RT_NULL;
                }
            break;
            case 0x05:
...
static rt_err_t _data_notify(udevice_t device, struct ep_msg* ep_msg)
{
    ...
    LOG_D("%s %x %d %d %d\r\n", __FUNCTION__, EP_ADDRESS(ep), device->state, ep->request.remain_size, EP_MAXPACKET(ep));

    if(EP_ADDRESS(ep) & USB_DIR_IN)
    {
        size = ep_msg->size;
      
        if(ep->request.remain_size >= EP_MAXPACKET(ep) && ep->request.remain_size != 0)
        {
        ...
        else if(ep->request.remain_size > 0 && ep->request.remain_size != 0)
        {
...
static rt_ssize_t rt_usbd_ep_write(udevice_t device, uep_t ep, void *buffer, rt_size_t size)
{
	...
	/* critical will trig write mutex error */
//    rt_enter_critical(); 
...
static rt_ssize_t rt_usbd_ep_read_prepare(udevice_t device, uep_t ep, void *buffer, rt_size_t size)
{
    ...
    #else
    return dcd_ep_read_prepare(device->dcd, EP_ADDRESS(ep), buffer, size);   //优化usb dma全长度读
    #endif
}
                              ...
                              ufunction_ops_t ops)
{
	...
	for(rt_uint8_t i = 0; i < sizeof(func->intf) / sizeof(func->intf[0]); i++)
    {
        func->intf[i] = RT_NULL;
    }
    
    for(rt_uint8_t i = 0; i < sizeof(func->ep) / sizeof(func->ep[0]); i++)
    {
        func->ep[i] = RT_NULL;
    }

    for(rt_uint8_t i = 0; i < sizeof(func->setting) / sizeof(func->setting[0]); i++)
    {
        func->setting[i] = RT_NULL;
    }    
...
rt_err_t rt_usbd_function_release(ufunction_t func)
{
    ...
    if(func == RT_NULL)
    {
        LOG_D("%s [%d]: invalid parameter!!!\n",__FUNCTION__,__LINE__);
        return -RT_ERROR;
    }
    
    len = sizeof(func->ep) / sizeof(func->ep[0]);    
    for(i = 0; i < len; i++)
    {
        if(func->ep[i] != RT_NULL)
        {
            rt_free(func->ep[i]);
            func->ep[i] = RT_NULL;
        }
    ...
    len = sizeof(func->setting) / sizeof(func->setting[0]);
    for(i = 0; i < len; i++)
    {
        if(func->setting[i] != RT_NULL)
        {
            if(func->setting[i]->desc != RT_NULL)
            {
                rt_free(func->setting[i]->desc);
                func->setting[i]->desc = RT_NULL;
            }

            rt_free(func->setting[i]);
            func->setting[i] = RT_NULL;
        }
    }

    len = sizeof(func->intf) / sizeof(func->intf[0]);
    for(i = 0; i < len; i++)
    {
        if(func->intf[i] != RT_NULL)
        {
            rt_free(func->intf[i]);
            func->intf[i] = RT_NULL;
        }
...
static rt_err_t rt_usbd_ep_assign(udevice_t device, uep_t ep)
{
    ...
    {
		/* hgic endpoint assign */
        if ((device->dcd->ep_pool[i].type > USB_EP_ATTR_TYPE_MASK) ||
...
rt_err_t rt_usbd_ep_unassign(udevice_t device, uep_t ep)
{
    ...
    while(device->dcd->ep_pool[i].addr != 0xFF)
    {
        if ((device->dcd->ep_pool[i].type > USB_EP_ATTR_TYPE_MASK) && (device->dcd->ep_pool[i].status == ID_ASSIGNED)) 
        {
            device->dcd->ep_pool[i].status = ID_UNASSIGNED;
...
{
    struct rt_messagequeue *usb_mq = parameter;
    while(1)
    {
        struct udev_msg msg;
        udevice_t device;
...
rt_err_t rt_usbd_event_signal(struct udev_msg* msg)
{
    ...
    return rt_mq_send(&msg->dcd->usb_mq, (void*)msg, sizeof(struct udev_msg));
}
...
#define usb_mq_pool_size ((USBD_MQ_MSG_SZ+sizeof(void*))*USBD_MQ_MAX_MSG)
